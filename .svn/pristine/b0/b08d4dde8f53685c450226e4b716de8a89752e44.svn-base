...
private List<Pointage> listpointage;
	...
	public void init() {
		...
		listPersonnel.addAll(getPersonnelService().findAllPersonnels()); // load
	...
	public void initializePointage() {
		...
		listMvt.addAll(getMvtService().findMvtByDates(startDate, endDate)); // load
																			// all
																			// mvt
																			// by
																			// interval
...
System.out.println("succés"+listpointage.size());
	...
	public Date FindE2(Personnel p, SequenceDetail d) {
		...
		Mvt mvt = null;
		Date tmp = d.getE2();
		int x = (int) ((d.getPause().getTime() / 1000 * 60 * 2));
		tmp.setHours(tmp.getHours() - x);
		System.out.println("E2 ==== " + tmp);
		Date tmp2 = d.getE2();
		tmp2.setHours(tmp2.getHours() + x);

		List<Mvt> listToscan = getmvtbyDate(p, tmp, d.getS2());
		if(listToscan.size()==0)
		{
			return null;
		}
	...
	public Date FindS1(Personnel p, SequenceDetail d) {
		...
		List<Mvt> listToscan = getmvtbyDate(p, tmp, d.getE2());
		if(listToscan.size()==0)
		{
			return null;
		}
		...
		if (d.getS1().before(d.getE1())) {
			...
			for (Mvt m : listToscan) {
				...
				if ((getdiffrence(d.getS1(), m.getHeure()) < dif)&&(m.getHeure().compareTo(d.getE2())!=0)) {
	...
	public Date FindS2(Personnel p, SequenceDetail d) {
		...
		List<Mvt> listToscan = getmvtbyDate(p, pointage.getE2(), tmp);
		if(listToscan.size()==0)
		{
			return null;
		}
	...
	public void loadRetard() {
		...
		if (pointage.getSequenceDetail() != null) {
			if ((pointage.getSequenceDetail().getE1() != null)&&(pointage.getE1() != null)) {
				Date dif = new Date();
				if (pointage.getSequenceDetail().getE1().before(pointage.getE1())) {
					System.out.println(pointage.getSequenceDetail().getE1()	+ "  pointé  " + pointage.getE1());
					dif.setTime((pointage.getE1().getTime()	- pointage.getSequenceDetail().getE1().getTime() - 1000 * 60 * 60));
			...
			if ((pointage.getSequenceDetail().getE2() != null)&&(pointage.getE2() != null)) {
				Date dif2 = new Date();
				if (pointage.getSequenceDetail().getE2().before(pointage.getE2())) {
					System.out.println(pointage.getSequenceDetail().getE2()	+ "  pointé  " + pointage.getE2());

					dif2.setTime(pointage.getE2().getTime()	- pointage.getSequenceDetail().getE2().getTime()- 1000 * 60 * 60);
...
hourstowork.setTime(dif.getTime()+dif2.getTime()-(60*60*1000));
pointage.setPresenceHrMn(hourstowork);
		}
	}

	
	public void loadHoursTowork() {
		if (pointage.getSequenceDetail() != null) {
			Date dif = new Date();
			Date dif2 = new Date();
